...
contract HapticDAO {
    ...
    modifier requirePayment(){
        require(msg.value == 0.01 ether, "Not enough paid");
        _;
    }

    function addDaoCard(string memory _title, string memory _description) requirePayment() uniqueTitle(_title) payable external returns(uint) {
    ...
    function increaseScore(string memory _title) requirePayment() payable external {
        titleToCardMap[_title].score+=1;
        emit ScoreChange("score increased", _title);
    }

    function decreaseScore(string memory _title) requirePayment() payable external {
        titleToCardMap[_title].score-=1;
    ...
    function getAmountCards() public view returns (uint) {
      return daoCards.length;
    }
...
// contract deployed at: 0x49Bf054E5Dc02998ECd4C31F94cAB38821c5983f